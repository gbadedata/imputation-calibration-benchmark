...
HM3=$RAW/w_hm3.snplist
REMOVALS=${REMOVALS:-}

for f in "$VCF" "$PANEL" "$HM3"; do
  [ -f "$f" ] || { echo "missing input: $f" >&2; exit 1; }
done
mkdir -p "$DER"

echo "== 1/5 biallelic SNP subset (phase-preserving) =="
bcftools view -m2 -M2 -v snps "$VCF" -Oz -o "$DER/chr20.bisnp.vcf.gz"
...
echo "== 4/5 site partition =="
python -m src.cli make-sites --hm3 "$HM3" --site-table "$DER/sites_all.tsv" --outdir "$DER"
